...
export default function HomeScreen() {
  ...
  const { data: options, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const mappedActividades = (response.actividades || []).map(
        (a: string) => ({
          label: a, // Usamos la palabra completa para mantener la seccion
          value: a,
        }),
      );
    ...
    // Configuración para que NO cargue a cada rato:
  ...
  const handleCalculate = async () => {
    if (!antiguedad || !departamento || !grado || !actividad) {
      Alert.alert(
        "Campos incompletos",
        "Uno o más campos están vacíos. Por favor complételos.",
        [{ text: "Entendido" }],
      );
      return;
    }
    ...
    if (isNaN(antiguedadNumber) || antiguedadNumber <= 0) {
      Alert.alert(
        "Dato inválido",
        "La antigüedad debe ser un número válido mayor a 0.",
        [{ text: "Entendido" }],
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Error",
        "Ocurrió un error al calcular los aranceles. Intente nuevamente.",
        [{ text: "Aceptar" }],
      );
    }
  };
  ...
  if (isProcessing) {
    return (
      <View className="flex-1 bg-[#0f3e33] items-center justify-center">
        <Stack.Screen options={{ headerShown: false }} />
        <ActivityIndicator size="large" color="#ffffff" />
        <Text className="text-white mt-6 font-bold uppercase tracking-[0.2em] text-[10px] text-center">
          Generando Desglose de Aranceles...
        </Text>
      </View>
    );
  }
  ...
  return (
    ...
    <SafeAreaView
      ...
      edges={["top"]}
    >
      {/*Mejora e la visibilidad del boton y ajuste de iconos de sistema para fondo verde*/}
      <StatusBar
      ...
      <View className="flex-1">
        ...
        >
          {/* Header Personalizado: bg-white en claro y verde institucional en oscuro. justify-center para alineación total. */}
          <View className="flex-row items-center justify-center bg-white dark:bg-[#0f3e33] p-4 border-b border-gray-200 dark:border-[#0f3e33] relative">
            <Pressable
              className="size-10 items-center justify-center rounded-full active:bg-gray-100 dark:active:bg-white/10 absolute left-2 z-20"
              onPress={() => router.back()}
            >
              {/* Cambiamos el color de la flecha a forma dinamica */}
              <MaterialIcons
                name="arrow-back-ios"
                size={20}
                color={Platform.OS === "ios" ? undefined : "#3c8d50"}
                className="dark:text-white"
              />
            </Pressable>

            <View className="px-2 items-center">
              <Text className="text-center text-lg font-bold leading-tight text-[#0f3e33] dark:text-white uppercase">
                Formulario de Cálculo
              </Text>
              <Text className="text-center text-[10px] uppercase tracking-widest text-accent dark:text-emerald-400 font-semibold mt-0.5">
                CICB Bolivia
              </Text>
            </View>
          </View>
              ...
              <>
                ...
                <View className="mb-2">
                  ...
                  <FormInput
                    ...
                    value={antiguedad}
                    onChangeText={(text) => {
                      //Evitar que los anos tengan mas de dos digitos
                ...
                <View className="flex-row gap-3 rounded-xl bg-primary/5 dark:bg-primary/20 p-4 border border-primary/10">
                  ...
                  <Text className="flex-1 text-sm leading-relaxed text-primary/80 dark:text-white/80">
                    Los Aranceles Profesionales se fundamentan en la Ley N° 1449
                    por lo tanto, son de cumplimiento obligatorio para la
                    población en general y para todos los profesionales{" "}
